...
// In order to not freeze the page, setTimeout is called regurlarly to let some CPU to other tasks.
// Every 10'000 computations, seems to be the right pace to call setTimeout.
Hashcash.Stamp.prototype.work = function(callback) {
  this.startClock()
  while (!this.check())
    if ((this.counter += 1) % 10000 == 0)
