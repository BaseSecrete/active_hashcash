...
// Tries WebGPU first for massive GPU parallelism (~30x faster). WebGPU is
// supported in all major browsers (Chrome 113+, Edge 113+, Firefox 141+,
// Safari 26+). Falls back to a synchronous pure JS SHA-256 tight loop for
// older browser versions. The worker is inlined as a Blob URL so no extra
...
Hashcash.Stamp.prototype.work = function(callback) {
  ...
  var workerCode = function() {
    // ==================================================================
    // Pure JS SHA-256 (FIPS 180-4) - used as fallback when WebGPU is
    // unavailable. Also used by WebGPU path for final verification.
    // ==================================================================
...
';
    ...
    // ==================================================================
    // Entry point: try WebGPU, fall back to Pure JS
    // ==================================================================
  ...
  worker.onmessage = function(e) {
    ...
    if (e.data.webgpuError) {
      console.warn("Hashcash WebGPU unavailable, using pure JS:", e.data.webgpuError)
      return
    }
    if (e.data.found) {
      Hashcash._worker = null
